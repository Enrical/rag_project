...
class RAGPipeline:
    ...
    def retrieve_chunks(self, query: str, scope: str = None) -> List[str]:
        ...
        payload = {
            "query": query,
            "filters": {
                "scope": scope  # Scope could now be "all documents" or specific categories
            }
        }
...
def initialize_session_state():
    ...
    if 'uploaded_documents' not in st.session_state:
        st.session_state.uploaded_documents = []  # Store metadata of uploaded documents
    if 'chat_history' not in st.session_state:
        st.session_state.chat_history = []


def main():
    st.set_page_config(page_title="Multi-Document Query System", layout="wide")
    initialize_session_state()

    st.title("📄 Multi-Document Query System")
    ...
    # Query Section
    if st.session_state.uploaded_documents:
        st.markdown("### 🔍 Query Across Documents")
